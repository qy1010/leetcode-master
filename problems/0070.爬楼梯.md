...
所以到<mark>第三层楼梯的状态可以由第二层楼梯 和 到第一层楼梯状态推导出来</mark>，那么就可以想到动态规划了。
...
首先是dp[i - 1]，上i-1层楼梯，有dp[i - 1]种方法，那么再一步跳一个台阶不就是dp[i]了么。<mark>我理解就是：dp[i - 1]都是每次跳一级的方式</mark>

还有就是dp[i - 2]，上i-2层楼梯，有dp[i - 2]种方法，那么再一步跳两个台阶不就是dp[i]了么。<mark>我理解就是：dp[i - 2]都是每次跳二级的方式</mark>

那么dp[i]就是 dp[i - 1]与dp[i - 2]之和！结合以上理解，才有有这公式

<mark>备注</mark>

这个状态公式由来就是如果我们想要跳上第`i`层楼梯，那我们只需要从`i-1` 层楼梯再往上跳一步或者从·`i-2` 层楼梯往上跳两步；

[疑问点]([力扣](https://leetcode-cn.com/problems/climbing-stairs/solution/wei-shi-yao-shi-dpidpi-1dpi-2-by-sevente-8bni/))：从`i-2`跳到`i`有两种方式

1. `1, 1`一步一步跳上去 （其实就是从`i-1`层楼梯往上跳一步）

2. `2` 一下跳2步

`所以dp[i] = dp[i - 1] + dp[i - 2] 。`

<mark>在推导dp[i]的时候，一定要时刻想着dp[i]的定义</mark>，否则容易跑偏。
