...
dp[i-1][j-weight[i]] : i-1: 没有放入i， j-weight[i], 没有放入i时，j的容量，为什么没有j没有更新j（背包）的容量呢，因为做了一次判断，就是 j < weight[i].

max(dp[i - 1][j], dp[i - 1][j - weight[i]] + value[i]); 为什么需要max，因为dp[i - 1][j - weight[i]] + value[i]代表，放下i后的背包的最大价值，因为前面判断了 j  < weight[i],所以这里一定能放得下i，但是放下了i后，容量就需要减少，[j- weight[i]], 我但是就想不明白为什么i一定能放得下，放下i后，之前的价格怎么取？dp[i - 1][j - weight[i]]就是代表放下i后，所剩下的容量的最大价值。

动态规划需要一步一步往下推，不能跳着思路想。因为不知道那一步的结果就需要往回找。所以才会有dp数组，从[0..i], 中间每一步都需要计算。它不像我日常思考一样跳着来。上述例子，我就直接思考背包4，所以我找物品相加出是4的价值，与物品刚好等于背包容量的价值来对比，就知道结果的，但是这中想法是无法转化成代码的！！这需要我什么注意

所以这里一定一定分清楚下标的含义。i是物品的下标，j是背包的容量。 i-1是上一个物品，j - weight[i] 是 j背包没有加入i物品的容量
