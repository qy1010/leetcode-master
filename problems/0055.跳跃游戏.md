...
<mark>而cover每次只取 max(该元素数值补充后的范围, cover本身范围)。</mark>
...
`cover = max(i + nums[i], cover)` cover是范围，在这个范围内逐个找，每次的范围 = i + nums[i] , nums[i] 的最大范围， `i`补充范围，这个的for循环终止条件是cover，是对于我

<mark>开始求算到cover后，while循环往回找的想法，用上述写法巧妙很多</mark>
