...
<mark>直觉上来看，貌似只射重叠最多的气球，用的弓箭一定最少，那么有没有当前重叠了三个气</mark><mark>球，我射两个，留下一个和后面的一起射这样弓箭用的更少的情况呢？</mark>

尝试一下<mark>举反例</mark>，发现没有这种情况。
...
判断两两是不是同一个区间，首先先对区间进行排序，方便操作。（数组排序后再运行是常规操作）。

1. 用`i`的左边界是否大于`i-1`的有边界判断是否在同一个区间

2. `min(i的右边界， i-1的右边界)`更新重叠的最小右边界
