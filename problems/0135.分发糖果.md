...
- 为什么第二次不能从前往后，需要从后往前？
  ...
  例如： 如果还是从前往后的话，比较完成下标是2的时候，下标为1的结果就不能更新了！
  
  | ratings  | 1   | 3              | 2              | 1   |
  |:--------:|:---:|:--------------:|:--------------:|:---:|
  | CandyVes | 1   | 2              | 1              | 1   |
  | 前->后     | 1   | <mark>2</mark> | <mark>2</mark> | 1   |

- 为什么需要取Max(candyVes[i], candyVes[i+1] + 1)，不能直接去candyVes[i+1] + 1？
  
  如果前一个从前往后得出来的数已经大于candyVes[i+1] + 1，见高亮部分，所以第二次从后往前就需要去max值
  
  | Ratings  | 1   | 2   | 3   | 4              | 2              | 1   | 1   |
  | -------- | --- | --- | --- | -------------- | -------------- | --- | --- |
  | CandyVes | 1   | 2   | 3   | 4              | 1              | 1   | 1   |
  | 后->前     | 1   | 2   | 3   | <mark>4</mark> | <mark>2</mark> | 1   | 1   |
