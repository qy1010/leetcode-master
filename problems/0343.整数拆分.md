...
<mark>备注：[来自Leetcode题解]([力扣](https://leetcode-cn.com/problems/integer-break/solution/zheng-shu-chai-fen-by-leetcode-solution/))</mark>

对于的正整数`n`,但`n>=2`时候，可以拆分成至少两个正整数的和，令`k`是拆分出来的第一个正整数，则剩下的部分是`n-k`,`n-k`可以不继续拆分，也可以继续拆分成至少两个正整数的和。由于每个正整数对应的最大乘积取决于比它小的正整数对应的最大乘积。因此可以使用动态规划来求解。

创建数组`dp`，其中`dp[i]`表示讲正整数i拆分成至少两个正整数的和之后，这些正整数的最大乘积。特别地，0不是正整数，1是最小的正整数，0和1不能拆分，因此`dp[0] = dp[1] = 0`

但`i >= 2`时，假设对正整数i拆分出第一个正整数是 `j（1 <= j <= i）`,则有一下两种方案：

- 将 `i` 拆分成 `j` 和 `i - j` 的和，且` i - j `不再拆分成多个正整数，此时的乘积是` j * (i - j)`

- 将` i `拆分成 `j` 和` i - j `的和， 且` i - j `继续拆分成多个正整数，此时的乘积是 `j * dp[i - j]`

因此，但 j 固定时，有 `dp[i] = max(j * (i - j ), j * dp[i - j])`。由于j 的取值范围是 1 到 i  - 1， 需要遍历所有的j 得到dp[i] 的最大值，因此可以得到状态转义方程

`        dp[i]= 1≤j<i max​{max(j×(i−j),j×dp[i−j])}    1≤j<i 为max的下标`
...
为什么dp[i] = max(dp[i], max((i - j) * j, dp[i - j] * j));  第一个max中是dp[i], 真开始时候没有赋值？？

max中的第一个dp[i]是真第二层循坏使用的。但是首次使用没有赋值默认是0？？(是0)
